...
export const StarBot = (app) => {
  ...
  bot.on('message', (msg) => {
    ...
    console.log(
      `User ${user.first_name} ${user.last_name} (@${user.username}, ${userChatID}) used the bot.`,
    );
  });
  ...
  bot.on('message', (msg) => {
    ...
    if (msg.text === '/lichtap') {
      bot.sendMessage(msg.chat.id, 'Do you like pizza?', {
  ...
  bot.on('callback_query', (query) => {
    if (query.data === 'hom nay') {
      bot.answerCallbackQuery(query.id, {
        text: 'hom nay ban khong co lich tap!',
      });
